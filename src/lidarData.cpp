...
/*Following change in func def was done to aid in highlighting prev and curr point considered for TTC calculation*/
//void showLidarImgOverlay(cv::Mat &img, std::vector<LidarPoint> &lidarPoints, cv::Mat &P_rect_xx, cv::Mat &R_rect_xx, cv::Mat &RT,LidarPoint minCurrPoint,LidarPoint minPrevPoint, cv::Mat *extVisImg)
void showLidarImgOverlay(cv::Mat &img, std::vector<LidarPoint> &lidarPoints, cv::Mat &P_rect_xx, cv::Mat &R_rect_xx, cv::Mat &RT, cv::Mat *extVisImg)
{
    ...
    /*Following changes were done to highlight last and current point
     * that are considered for TTC calculation*/
    /*
    X.at<double>(0, 0) = minPrevPoint.x;
	X.at<double>(1, 0) = minPrevPoint.y;
	X.at<double>(2, 0) = minPrevPoint.z;
	X.at<double>(3, 0) = 1;
	...
	cv::circle(overlay, pt, 5, cv::Scalar(255, 0,0), -1);

	X.at<double>(0, 0) = minCurrPoint.x;
	X.at<double>(1, 0) = minCurrPoint.y;
	X.at<double>(2, 0) = minCurrPoint.z;
	X.at<double>(3, 0) = 1;
